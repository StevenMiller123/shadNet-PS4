...
static s32 sceNpScoreCreateNpTitleCtx_hook(OrbisNpServiceLabel serviceLabel,
    ...
    return Libraries::Np::NpScore::sceNpScoreCreateNpTitleCtx(serviceLabel, selfNpId);
}

HOOK_INIT(sceNpScoreGetFriendsRanking);
static s32 sceNpScoreGetFriendsRanking_hook(s32 reqId, OrbisNpScoreBoardId boardId, s32 includeSelf,
    ...
    return Libraries::Np::NpScore::sceNpScoreGetFriendsRanking(
        reqId, boardId, includeSelf, rankArray, rankArraySize, commentArray, commentArraySize,
...
static s32 sceNpScoreGetGameData_hook(s32 reqId, OrbisNpScoreBoardId boardId, const OrbisNpId* npId,
                                      u64* totalSize, u64 recvSize, void* data, void* option) {
    return Libraries::Np::NpScore::sceNpScoreGetGameData(reqId, boardId, npId, totalSize, recvSize,
                                                         data, option);
}

HOOK_INIT(sceNpScoreGetGameDataAsync);
static s32 sceNpScoreGetGameDataAsync_hook(s32 reqId, OrbisNpScoreBoardId boardId,
                                           const OrbisNpId* npId, u64* totalSize, u64 recvSize,
                                           void* data, void* option) {
    return Libraries::Np::NpScore::sceNpScoreGetGameDataAsync(reqId, boardId, npId, totalSize,
                                                              recvSize, data, option);
}

HOOK_INIT(sceNpScoreGetRankingByNpId);
static s32 sceNpScoreGetRankingByNpId_hook(s32 reqId, OrbisNpScoreBoardId boardId,
    ...
    return Libraries::Np::NpScore::sceNpScoreGetRankingByNpId(
        reqId, boardId, npIdArray, npIdArraySize, rankArray, rankArraySize, commentArray,
...
static s32 sceNpScoreGetRankingByRange_hook(s32 reqId, OrbisNpScoreBoardId boardId,
...
static s32 sceNpScoreAbortRequest_hook(s32 reqId) {
    return Libraries::Np::NpScore::sceNpScoreAbortRequest(reqId);
}

HOOK_INIT(sceNpScoreCensorComment);
static s32 sceNpScoreCensorComment_hook(s32 reqId, const char* comment, void* option) {
    return Libraries::Np::NpScore::sceNpScoreCensorComment(reqId, comment, option);
}

HOOK_INIT(sceNpScoreCensorCommentAsync);
static s32 sceNpScoreCensorCommentAsync_hook(s32 reqId, const char* comment, void* option) {
    return Libraries::Np::NpScore::sceNpScoreCensorCommentAsync(reqId, comment, option);
}
...
static s32 sceNpScoreCreateNpTitleCtxA_hook(OrbisNpServiceLabel npServiceLabel,
                                            UserService::OrbisUserServiceUserId selfId) {
    return Libraries::Np::NpScore::sceNpScoreCreateNpTitleCtxA(npServiceLabel, selfId);
}

HOOK_INIT(sceNpScoreCreateRequest);
static s32 sceNpScoreCreateRequest_hook(s32 titleCtxId) {
    return Libraries::Np::NpScore::sceNpScoreCreateRequest(titleCtxId);
}
...
static s32 sceNpScoreDeleteNpTitleCtx_hook(s32 titleCtxId) {
    return Libraries::Np::NpScore::sceNpScoreDeleteNpTitleCtx(titleCtxId);
}

HOOK_INIT(sceNpScoreDeleteRequest);
static s32 sceNpScoreDeleteRequest_hook(s32 reqId) {
    return Libraries::Np::NpScore::sceNpScoreDeleteRequest(reqId);
}

HOOK_INIT(sceNpScoreGetBoardInfo);
static s32 sceNpScoreGetBoardInfo_hook(s32 reqId, OrbisNpScoreBoardId boardId,
                                       OrbisNpScoreBoardInfo* boardInfo, void* option) {
    return Libraries::Np::NpScore::sceNpScoreGetBoardInfo(reqId, boardId, boardInfo, option);
}

HOOK_INIT(sceNpScoreGetBoardInfoAsync);
static s32 sceNpScoreGetBoardInfoAsync_hook(s32 reqId, OrbisNpScoreBoardId boardId,
                                            OrbisNpScoreBoardInfo* boardInfo, void* option) {
    return Libraries::Np::NpScore::sceNpScoreGetBoardInfoAsync(reqId, boardId, boardInfo, option);
}

HOOK_INIT(sceNpScoreGetFriendsRankingA);
static s32 sceNpScoreGetFriendsRankingA_hook(s32 reqId, OrbisNpScoreBoardId boardId,
...
static s32 sceNpScoreGetGameDataByAccountId_hook(s32 reqId, OrbisNpScoreBoardId boardId,
    ...
    return Libraries::Np::NpScore::sceNpScoreGetGameDataByAccountId(
        reqId, boardId, accountId, totalSize, recvSize, data, option);
}

HOOK_INIT(sceNpScoreGetGameDataByAccountIdAsync);
static s32 sceNpScoreGetGameDataByAccountIdAsync_hook(s32 reqId, OrbisNpScoreBoardId boardId,
                                                      OrbisNpAccountId accountId, u64* totalSize,
                                                      u64 recvSize, void* data, void* option) {
...
static s32 sceNpScoreRecordScore_hook(s32 reqId, OrbisNpScoreBoardId boardId,
    ...
    return Libraries::Np::NpScore::sceNpScoreRecordScore(reqId, boardId, score, scoreComment,
                                                         gameInfo, tmpRank, compareDate, option);
}

HOOK_INIT(sceNpScoreRecordScoreAsync);
static s32 sceNpScoreRecordScoreAsync_hook(s32 reqId, OrbisNpScoreBoardId boardId,
                                           OrbisNpScoreValue score,
                                           const OrbisNpScoreComment* scoreComment,
                                           const OrbisNpScoreGameInfo* gameInfo,
                                           OrbisNpScoreRankNumber* tmpRank,
    ...
    return Libraries::Np::NpScore::sceNpScoreRecordScoreAsync(
        reqId, boardId, score, scoreComment, gameInfo, tmpRank, compareDate, option);
}

HOOK_INIT(sceNpScoreSanitizeComment);
static s32 sceNpScoreSanitizeComment_hook(s32 reqId, const char* comment, char* sanitizedComment,
                                          void* option) {
    return Libraries::Np::NpScore::sceNpScoreSanitizeComment(reqId, comment, sanitizedComment,
                                                             option);
}

HOOK_INIT(sceNpScoreSanitizeCommentAsync);
static s32 sceNpScoreSanitizeCommentAsync_hook(s32 reqId, const char* comment,
                                               char* sanitizedComment, void* option) {
    return Libraries::Np::NpScore::sceNpScoreSanitizeCommentAsync(reqId, comment, sanitizedComment,
                                                                  option);
}

HOOK_INIT(sceNpScoreSetPlayerCharacterId);
static s32 sceNpScoreSetPlayerCharacterId_hook(s32 ctxId, OrbisNpScorePcId pcId) {
    return Libraries::Np::NpScore::sceNpScoreSetPlayerCharacterId(ctxId, pcId);
}

HOOK_INIT(sceNpScoreSetThreadParam);
static s32 sceNpScoreSetThreadParam_hook(s32 threadPriority, u64 cpuAffinityMask) {
    return Libraries::Np::NpScore::sceNpScoreSetThreadParam(threadPriority, cpuAffinityMask);
}

HOOK_INIT(sceNpScoreSetTimeout);
static s32 sceNpScoreSetTimeout_hook(s32 id, s32 resolveRetry, s32 resolveTimeout, s32 connTimeout,
                                     s32 sendTimeout, s32 recvTimeout) {
    return Libraries::Np::NpScore::sceNpScoreSetTimeout(id, resolveRetry, resolveTimeout,
                                                        connTimeout, sendTimeout, recvTimeout);
}

HOOK_INIT(sceNpScoreWaitAsync);
static s32 sceNpScoreWaitAsync_hook(s32 reqId, s32* result) {
    return Libraries::Np::NpScore::sceNpScoreWaitAsync(reqId, result);
}
